...
class clientManager(object):
    ...
    def isAvailable(self, clientId, cur_time, time_window, time_slots, lookup_availability_periods=1):
        ...
        if lookup_availability_periods == 1:
            start, end = availabilityPeriods[0]
    ...
    def getPriority(self,clientId, cur_time, time_window, lookup_timeslots=1):
        priority=0
        for i in range(lookup_timeslots, 0, -1):
            logging.info('HERE')
            if self.isAvailable(clientId, cur_time, time_window, i):
                priority = lookup_timeslots - i
                logging.info('==== Client {} - priority {} '.format(clientId, priority))
                break
        return priority
    ...
    def registerClient(self, hostId, clientId, size, speed, possible_bandwidths, duration=1):
        ...
        self.Clients[uniqueId] = Client(hostId, clientId, speed, possible_bandwidths, user_trace)
    ...
    def get_completion_time_with_variable_network(self, client_id, batch_size, local_steps, upload_size, download_size):
        try:
            return self.Clients[self.getUniqueId(0, client_id)].get_completion_time_with_variable_network(
    ...
    def getClientFeasibilityForParticipation(self, client_id, cur_time, upload_size, download_size, random_client_quota = False, deadline = 0):
        try:
            if self.user_trace is None:
                ...
                if self.Clients[self.getUniqueId(0, client_id)].isActive(cur_time):
                    #Faraz - 2. Check if client will be able to meet the deadline
                    compute_time = self.Clients[self.getUniqueId(0, client_id)].completion_time['computation']
                    communication_time = self.Clients[self.getUniqueId(0, client_id)].completion_time['communication']
                    #Faraz - 3. Check if client can deliver within the deadline and before it becomes inactive
                    (will_participate, deadline_difference) = self.Clients[self.getUniqueId(0, client_id)].can_participate(cur_time, compute_time, communication_time)
    ...
    def resampleClients(self, numOfClients, cur_time=0, time_window=0):
        ...
        if self.args.avail_priority >= 1:
            ...
            for c in feasible_clients:
                priority_vals[c] = self.getPriority(c, cur_time, time_window, lookup_timeslots=1)
