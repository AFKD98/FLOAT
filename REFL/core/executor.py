...
class Executor(object):
    ...
    def init_data(self):
        ...
        validation_sets = DataPartitioner(data=train_dataset, numOfClass=self.args.num_class, isVal=True)
        validation_sets.partition_data_helper(num_clients=num_clients, data_map_dir=self.args.data_map_dir)
    ...
    def validation_handler(self, client_id):
        ...
        try:
            ...
            data_loader = select_dataset(client_id, self.validation_sets, batch_size=args.test_bsz, isVal=True, collate_fn=self.collate_fn) #, seed=self.this_rank)
            ...
            test_loss, acc, acc_5, test_results = test_model(self.this_rank, self.model, data_loader, device=device, criterion=criterion, tokenizer=self.tokenizer, isVal=True)
            # logging.info(f"test_results: {test_results}")
            # logging.info(f"Validation time: {time.time() - evalStart} seconds")
    ...
    def Val(self, val_config):
        ...
        """
        try:
            clients = [self.this_rank]
            round = 0
            if 'clients' in val_config:
                clients = val_config['clients']
            if 'round' in val_config:
                round = val_config['round']
            # logging.info(f"Faraz - Validation clients: {clients}")
            # logging.info(f"Faraz - Validation round: {round}")
