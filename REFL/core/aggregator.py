...
class Aggregator(object):
    ...
    def print_stats(self, clients_to_run):
        ...
        try:
            logging.info(f"Round {self.epoch}:")
            logging.info(f"Wall clock time: {round(self.global_virtual_clock)}")
            logging.info(f"Selected participants to run: {clients_to_run}")
            logging.info(f"Number of succeeded participants: {len(clients_to_run)}")
            logging.info('Number of sampled participants: {}'.format(len(self.sampled_participants)))
            logging.info(f"Resource wastage of dropped clients: {self.dropped_clients_resource_usage_per_round[self.epoch]}")
            logging.info(f"Resource durations of dropped clients: {self.dropped_clients_resource_durations_per_round[self.epoch]}")
            logging.info(f"Number of clients dropped out: {len(self.clients_dropped_out)}")
            logging.info("round: {}, clients dropped out: {}".format(self.epoch, self.clients_dropped_out))
            logging.info("round: {}, stragglers: {}".format(self.epoch, self.round_stragglers))
            logging.info("round: {}, number of stragglers: {}".format(self.epoch, len(self.round_stragglers)))
            logging.info("round: {}, total resource usage of clients: {}".format(self.epoch, self.total_resources_selected_clients))
            logging.info('Real concurrent clients: {}'.format(len(clients_to_run) - len(self.clients_dropped_out_per_round[self.epoch])))
            logging.info('Virtual concurrent clients: {}'.format(len(clients_to_run) + len(self.clients_dropped_out_per_round[self.epoch])))
            logging.info(f"Deadline differences of clients: {self.deadline_differences_per_round[self.epoch]}")
            if len(self.deadline_differences_per_round[self.epoch]) > 0:
                logging.info(f"Mean of deadline differences: {np.mean(self.deadline_differences_per_round[self.epoch])}")
                logging.info(f"Std of deadline differences: {np.std(self.deadline_differences_per_round[self.epoch])}")
    ...
    def executor_info_handler(self, executorId, info):
        ...
        if self.registered_executor_info == self.num_executors:
            ...
            for index, _size in enumerate(info['size']):
                ...
                if clientId not in self.client_participation_rate:
                    self.client_participation_rate[clientId] = 0
                    
                self.client_manager.registerClient(executorId, clientId, size=_size, speed=systemProfile, possible_bandwidths=cluster_bandwidths)
    ...
    def tictak_client_tasks(self, sampled_clients, num_clients_to_collect):
        ...
        for client_to_run in sampled_clients:
            ...
            exe_cost = self.client_manager.get_completion_time_with_variable_network(client_to_run, batch_size=args.batch_size, local_steps=args.local_steps, upload_size=self.model_update_size, download_size=self.model_update_size)
            ...
            #Faraz - temporarily adding True to induce no dropouts
    ...
    def client_completion_handler(self, results): #, importance=1.0):
        ...
        if self.round_update and not isStale and not args.send_delta:
            logging.info('======== Aggregator aggregating updates')
    ...
    def round_completion_handler(self, importance=1.0):
        ...
        if str(args.wandb_tags).startswith('safa') and False: #args.exp_type == 4:
        ...
        if self.epoch not in self.clients_dropped_out_per_round:
            self.clients_dropped_out_per_round[self.epoch] = []
            self.dropped_clients_resource_usage_per_round[self.epoch] = []
            self.dropped_clients_resource_durations_per_round[self.epoch] = []
            self.deadline_differences_per_round[self.epoch] = []
        ...
        if self.args.stale_update != 0 or self.args.prohibit_reselect != 0:
            ...
            logging.info(f'====== Aggreagator REMOVE: total {count} clients')
            try:
                while len(re_sel_client) > 0:
                    c = re_sel_client.pop()
                    #online_clients, _ = self.client_manager.getOnlineClients(cur_time=self.global_virtual_clock)
                    #Faraz - setting this to avoid empty online clients
                    logging.info(f'====== Faraz 0')
                    # self.client_manager.cur_online_clients = self.client_manager.getFeasibleClients(cur_time=self.global_virtual_clock)
                    index = self.rng.randint(0, len(self.client_manager.cur_online_clients) - 1)
                    logging.info(f'====== Faraz 1')
                    cid = self.client_manager.cur_online_clients[index]
                    trials = 10
                    while cid == c or cid in clientsToRun or cid in self.staleWeights:
                        logging.info(f'====== Faraz 2')
                        index = self.rng.randint(0, len(self.client_manager.cur_online_clients) - 1)
                        cid = self.client_manager.cur_online_clients[index]
                        trials -= 1
                        logging.info(f'====== Faraz 3')
                        
                    if trials >= 0:
                        replacement_clients.append(cid)
                        clientsToRun.append(cid)
                        virtual_client_clock[cid] = self.client_manager.get_completion_time_with_variable_network(cid, batch_size=args.batch_size, local_steps=args.local_steps, upload_size=self.model_update_size, download_size=self.model_update_size)
                        # virtual_client_clock[cid] = self.client_manager.get_completion_time_with_variable_network(cid)
                        logging.info(f'====== Aggreagator ADD: client {cid} to replace {c}')
                    logging.info(f'====== Aggreagator REMOVE: reselect client {c} to {cid}')
            except Exception as e:
                logging.info(f'====== ERROR -- Aggreagator REMOVE: error {e}')
        ...
        if str(args.wandb_tags).startswith('safa') and not self.round_update and args.stale_update != 0 and False:
        ...
        #Faraz - add the client to the list of dropped out clients
        try:
            for client_to_run in clientsToRun:
                self.total_resources_selected_clients.append(virtual_client_clock[client_to_run])
                #Faraz - temporarily adding False to induce no dropouts
                if not self.client_manager.isClientActive(client_to_run, self.global_virtual_clock, round_duration):
                    self.clients_dropped_out.append(client_to_run)
                    self.clients_dropped_out_per_round[self.epoch].append(client_to_run)
                    self.dropped_clients_resource_usage.append(client_to_run)
                    self.dropped_clients_resource_durations_per_round[self.epoch].append(virtual_client_clock[client_to_run])
                    will_participate, deadline_difference = self.client_manager.getClientFeasibilityForParticipation(client_to_run, self.global_virtual_clock, self.model_update_size, self.model_update_size, random_client_quota = False, deadline = 0)
                    logging.info("Client {} dropped out. Will participate: {} deadline difference: {}".format(client_to_run, will_participate, deadline_difference))
                    self.deadline_differences.append(deadline_difference)
                    self.deadline_differences_per_round[self.epoch].append(deadline_difference)
                    clientsToRun.remove(client_to_run)
        except Exception as e:
            logging.info("Error in adding dropped out clients: {}".format(e))
        ...
        #Faraz - printing stats
        self.print_stats(clientsToRun)
        ...
        while True:
            if len(self.event_queue) != 0:
                ...
                if event_msg == 'update_model':
                    ...
                    self.broadcast_msg(send_msg)
                elif event_msg == 'validate':
                    self.broadcast_msg({'event': 'validate', 'clients':self.client_manager.getAllClients(), 'round': self.epoch})
                ...
                if event_msg == 'train_nowait':
                    ...
                    logging.info(f'Round {self.epoch} len(self.stats_util_accumulator) {len(self.stats_util_accumulator)} total_clients {total_clients}')
                    if len(self.stats_util_accumulator) == total_clients or len(self.stats_util_accumulator) == len(self.clients_to_run):
