...
class Aggregator(job_api_pb2_grpc.JobServiceServicer):
    ...
    def print_stats(self, clients_to_run):
        ...
        try:
            logging.info(f"Round {self.round} stats:")
            ...
            logging.info("round: {}, clients dropped out: {}".format(self.round, self.clients_dropped_out_per_round))
    ...
    def update_client_dropped_out(self, client_id, exe_cost, deadline_difference, client_resources):
        try:
            ...
            self.dropped_clients_resource_usage_per_round[self.round].append({'clients': client_id, 'execution_durations': exe_cost, 'client_resources': client_resources})
        except Exception as e:
            logging.error(e)
            raise e
